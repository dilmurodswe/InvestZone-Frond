...
/* eslint-disable react-x/no-clone-element */
...
import { cloneElement, createContext, use, useEffect, useState } from "react"
...
export const CodeBlock = ({
    ...
    return (
        <CodeBlockContext value={{ value, onValueChange, data }}>
            <div
                className={cn(
...
export const CodeBlockFiles = ({
    ...
    const { data } = use(CodeBlockContext)
...
export const CodeBlockFilename = ({
    ...
    const { value: activeValue } = use(CodeBlockContext)
...
export const CodeBlockSelect = (props: CodeBlockSelectProps) => {
    const { value, onValueChange } = use(CodeBlockContext)
...
export const CodeBlockSelectContent = ({
    ...
    return (
        <SelectContent {...(props as any)}>{data.map(children)}</SelectContent>
    )
}
...
export const CodeBlockCopyButton = ({
    ...
    const { data, value } = use(CodeBlockContext)
...
export const CodeBlockContent = ({
    ...
    if (!syntaxHighlighting || isLoading) {
        return (
            <CodeBlockFallback {...(props as any)}>
                {children}
            </CodeBlockFallback>
        )
    }
