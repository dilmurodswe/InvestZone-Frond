...
import { RollStripsPrintModal } from "./roll-strips-print-modal"
...
function StripsButton({ onClick }: { onClick: () => void }) {
    return (
        <button
            type="button"
            onClick={onClick}
            className="inline-flex items-center justify-center gap-1 h-7 px-2.5 rounded-md border border-primary/40 text-primary hover:bg-primary/5 text-xs font-medium transition-colors"
            title="Открыть штрипсы этого рулона"
        >
...
function ManufacturePlanFactContent() {
    ...
    // Reference number of the roll whose штрипс-print modal is open (null = closed)
    const [stripsRef, setStripsRef] = useState<string | null>(null)
    ...
    return (
        <div className="flex flex-col gap-6">
            ...
            {stripsRef != null && (
                <RollStripsPrintModal
                    open={stripsRef != null}
                    onOpenChange={(v) => !v && setStripsRef(null)}
                    manufactureId={detail.id}
                    referenceNumber={stripsRef}
                    detail={detail}
                />
            )}
        </div>
